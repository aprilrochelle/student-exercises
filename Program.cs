  ...
  class Program
  {
    static void Main(string[] args)
    {
      ...
      db.Query<Cohort, Instructor, Student, Cohort>(@"
            ...
            {
              if (!cohortMembers.ContainsKey(cohort.Id))
              {
                cohortMembers[cohort.Id] = cohort;
              }
              cohortMembers[cohort.Id].Instructors.Add(instructor);
              cohortMembers[cohort.Id].Students.Add(student);
              return cohort;
            });
       ...
       */
      foreach (KeyValuePair<int, Cohort> member in cohortMembers)
      {
        List<string> instructorList = new List<string>();
        member.Value.Instructors.ForEach(i => instructorList.Add($"{i.FirstName} {i.LastName}"));

        List<string> studentList = new List<string>();
        member.Value.Students.ForEach(s => studentList.Add($"{s.FirstName} {s.LastName}"));

        Console.WriteLine($"{member.Value.Name} Instructors: {String.Join(", ", instructorList)}");
      ...
      Dictionary<int, Exercise> exercisesByStud = new Dictionary<int, Exercise>();
      ...
      {
        if (!exercisesByStud.ContainsKey(exercise.Id))
        {
          exercisesByStud[exercise.Id] = exercise;
        }
        cohort.Instructors.Add(instructor);
        student.Cohort = cohort;
        exercisesByStud[exercise.Id].AssignedStudents.Add(student);
        return exercise;
      });

      /*
          Display the exercises by student
       */

      foreach (KeyValuePair<int, Exercise> exe in exercisesByStud)
      {
        List<string> studentNames = new List<string>();

        List<string> instructorNames = new List<string>();

        foreach (Student s in exe.Value.AssignedStudents)
        {
            s.Cohort.Instructors.ForEach(i => instructorNames.Add($"{i.FirstName} {i.LastName}"));
        }


        exe.Value.AssignedStudents.ForEach(s => studentNames.Add($"{s.FirstName} {s.LastName} in {s.Cohort.Name} is working on {exe.Value.Name}, assigned by {String.Join(", ", instructorNames)}"));
